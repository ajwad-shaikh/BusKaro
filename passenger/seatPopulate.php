<?php
    ...
    if($result->num_rows == 0)
    {
    	$sql1="SELECT BID, RID, Capacity FROM buskaro.bus_instances WHERE BusDate = CURDATE()";
    ...
    if($result->num_rows == 0)
    {
    	$sql1="SELECT BID, RID, Capacity FROM buskaro.bus_instances WHERE BusDate = CURDATE() + INTERVAL 1 DAY";
    	$result1 = $conn->query($sql1);
    	echo $conn->error;
    	if($result1->num_rows > 0)
    	{
